...
from DB_offline.models import EllipseORM
...
class Ellipse(QGraphicsEllipseItem):
    ...
    def __init__(self, x: float, y: float, width: float, height: float,
                 ...
                 ellipse_id_pk: int = None, scheme_id_fk: int = None):
        ...
        self.ellipse_id_pk = ellipse_id_pk
        ...
        # Если эллипс имеет id_pk (то есть он загружен из БД), то при его удалении со сцены, устанавливается этот флаг.
        # И при проходе циклом по эллипсам, которые хранятся в списке эллипсов сцены, при сохранении плейбука
        # обновляются ORM-объекты, при этом эллипсы с этим флагом удаляются из списка сцены, из ORM и затем из БД.
    ...
    def mousePressEvent(self, event):
        ...
        if self.scene().mode == Modes.erase and event.button() == Qt.LeftButton:
            ...
            if self.ellipse_id_pk:
                self.is_deleted = True
            else:
                self.scene().ellipses.remove(self)
    ...
    def mouseDoubleClickEvent(self, event):
        if self.scene().mode == Modes.move:
            dialog = DialogFigureSettings(Modes.ellipse, self.border, self.border_color, self.border_width,
    ...
    def __eq__(self, other):
        return self.ellipse_id_pk == other.ellipse_id_pk if isinstance(other, EllipseORM) else super().__eq__(other)

    def __repr__(self):
        return f'\n\t<{self.__class__.__name__} (id_pk: {self.ellipse_id_pk}; scheme_id_fk: {self.scheme_id_fk};' \
    ...
    def return_data(self):
        return self.ellipse_id_pk, self.scheme_id_fk, self.x(), self.y(), self.rect().width(), self.rect().height(),\
