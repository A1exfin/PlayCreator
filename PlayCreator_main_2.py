...
from Dialog_windows import DialogNewPlaybook, DialogAbout, DialogLogIn, DialogSignUp,\
    DialogFirstTeamPlayerSettings, DialogSecondTeamPlayerSettings, DialogPlayerSettings
...
class PlayCreator(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.lineEdit_yards.textChanged.connect(lambda text: getattr(self, f'check_max_yards_{self.playbook.type}')(text))
        self.pushButton_place_first_team.clicked.connect(lambda: getattr(self, f'place_first_team_{self.playbook.type}')())
        self.pushButton_add_additional_off_player.clicked.connect(lambda: getattr(self, f'place_additional_offence_player_{self.playbook.type}')())
        self.pushButton_place_second_team.clicked.connect(lambda: getattr(self, f'place_second_team_{self.playbook.type}')())
        ...
        # result = dialog.exec()
        # dialog = DialogPlayerSettings(self.dialog_windows_text_color, 'C', '#000000', '#000000', parent=self)
        # dialog.exec()
        ...
        def new_playbook_dialog():
            ...
            if result == 1 and len(playbook_name) > 0:
                if dialog.radio_button_football.isChecked():
                    self.playbook = Playbook(playbook_name, 'football', False)
                elif dialog.radio_button_flag.isChecked():
                    self.playbook = Playbook(playbook_name, 'flag', True)
    ...
    def set_gui_for_playbook(self):
        ...
        if self.playbook.type == 'football':
    ...
    def set_gui_first_team_placed(self):
        ...
        if (self.playbook.type == 'football' and self.comboBox_team_type.currentIndex() == 0) or self.playbook.type == 'flag':
    ...
    def add_new_scheme(self):
        ...
        item = CustomListItem(Field(self, self.field_data, self.playbook.type), '')
    ...
    def create_first_team_players_football(self, players: tuple | list):
        for i, player in enumerate(players):
            ...
            if i == 10 and player[0] == 'punt_kick' and int(self.lineEdit_yards.text()) >= 95:
    ...
    def create_second_team_football(self, players: tuple | list):
        for i, player in enumerate(players):
            ...
            elif team == 'field_goal_def' and i == 10 and int(self.lineEdit_yards.text()) > 20:  # kick returner
                item = SecondTeamPlayer(team, position, text_color, border_color, symbol, x,
                                        self.current_scene.field_data.football_five_yard)
            elif team == 'defence' and i == 10 and int(self.lineEdit_yards.text()) < 3:  # free safety
                item = SecondTeamPlayer(team, position, text_color, border_color, symbol, x,
                                        y + self.get_yards_to_end_zone_football() + 3 * self.current_scene.field_data.football_one_yard)
            elif team == 'kick_ret' and 4 < i <= 7 and int(self.lineEdit_yards.text()) == 75:  # second line
                item = SecondTeamPlayer(team, position, text_color, border_color, symbol, x,
                                        y + self.get_yards_to_end_zone_football() - self.field_data.football_five_yard)
            elif team == 'kick_ret' and 7 < i <= 9 and int(self.lineEdit_yards.text()) == 75:  # third line
                item = SecondTeamPlayer(team, position, text_color, border_color, symbol, x,
    ...
    def delete_second_team(self):
        if self.current_scene.second_team_placed:
            ...
            if self.current_scene.allow_painting and \
                    (self.current_scene.current_player.team == 'defence'  # Используется для команды защиты в футболе и во флаге
